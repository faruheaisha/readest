...
| M3 — Content + Context Panel | Partial | Readest selection conversion, stable source-version identity, Locator context, responsive Context Panel, local learning saves, canonical Lexeme/Form/Sense/Expression/Occurrence persistence, BookNote-backed Annotation ownership, dictionary/translation/AI Action and Artifact flows, and source return exist. Context Panel note composition, dictionary-selected Sense enrichment, and general re-anchoring remain. Rich dictionary and annotation interactions continue through the original Readest surfaces. |
...
| `3ffeb3f4` | BookNote-backed Annotation port, stable content-version identity, tombstone preservation, and unused-table retirement |
...
- 16 learning test files, 58 tests passed, including canonical lexical sharing, Annotation round trips, tombstone preservation, concurrent writes, stable source-version identity, and legacy migrations.
...
Annotation is also one writable truth for the Readest-based MVP. `AnnotationRepositoryPort` now projects and updates the existing `BookConfig.booknotes` records used by the reader, notebook, import/export, file sync, BookOrbit, KOReader, Readwise, Notion, and Hardcover integrations. Adapter writes retain Readest presentation/sync metadata and deletion tombstones. The unused `learning_annotations` table is retired to `learning_annotations_legacy` so unexpected rows survive without remaining an active application store. Content-version identity now derives from the source file hash rather than mutable config timestamps or edition-group metadata.
